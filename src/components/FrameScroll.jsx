...
const FrameScroll = () => {
  ...
  useEffect(() => {
    ...
    const scrollAnimation = gsap.to({}, {
      scrollTrigger: {
        ...
        start: 'top top',
        end: '+=400%', // 4 screens worth of scroll for 200 frames
        pin: true,
  ...
  return (
    <section id="home" ref={containerRef} className="relative h-screen w-full bg-white overflow-hidden">
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-bounce opacity-40">
        <span className="text-[10px] uppercase tracking-widest font-semibold text-premium-black">
          Scroll to explore
        </span>
        <div className="w-[1px] h-12 bg-premium-black"></div>
      </div>
    </section>
  );
};
